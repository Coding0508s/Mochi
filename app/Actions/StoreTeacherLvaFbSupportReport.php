...
class StoreTeacherLvaFbSupportReport
{
    /**
     * @param  array<string, mixed>  $data
     */
    public function execute(int $teacherId, array $data, User $user): TeacherLvaFbSupportReport
    {
        ...
        return DB::transaction(function () use ($teacher, $validated, $user, $status, $markCompleted): TeacherLvaFbSupportReport {
            ...
            if ($markCompleted) {
                ...
                $supportRecord = SupportRecord::query()->create([
                    ...
                    'Support_Type' => config('coach_teacher_lva_fb.support_type_label'),
    ...
    private function authorize(Teacher $teacher, User $user): void
    {
        ...
        if (! $query->exists()) {
            throw new AuthorizationException('이 교사에 대한 LVA+FB 보고서를 작성할 권한이 없습니다.');
        }
    }
