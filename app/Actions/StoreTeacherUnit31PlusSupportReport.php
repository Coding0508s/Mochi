...
class StoreTeacherUnit31PlusSupportReport
{
    /**
     * @param  array<string, mixed>  $data
     */
    public function execute(int $teacherId, array $data, User $user): TeacherUnit31PlusSupportReport
    {
        ...
        return DB::transaction(function () use ($teacher, $validated, $user, $status, $markCompleted): TeacherUnit31PlusSupportReport {
            ...
            if ($markCompleted) {
                ...
                $supportRecord = SupportRecord::query()->create([
                    ...
                    'Support_Type' => config('coach_teacher_unit31_plus.support_type_label'),
                ...
                TeacherSupportSlotSync::apply(
                    ...
                    (string) config('coach_teacher_unit31_plus.support_type_label'),
                );
            }

            return TeacherUnit31PlusSupportReport::query()->create([
    ...
    private function authorize(Teacher $teacher, User $user): void
    {
        ...
        if (! $query->exists()) {
            throw new AuthorizationException('이 교사에 대한 Unit 31+ 보고서를 작성할 권한이 없습니다.');
        }
    }
