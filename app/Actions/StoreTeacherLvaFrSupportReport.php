...
class StoreTeacherLvaFrSupportReport
{
    /**
     * @param  array<string, mixed>  $data
     */
    public function execute(int $teacherId, array $data, User $user): TeacherLvaFrSupportReport
    {
        ...
        return DB::transaction(function () use ($teacher, $validated, $user, $status, $markCompleted): TeacherLvaFrSupportReport {
            ...
            if ($markCompleted) {
                ...
                $supportRecord = SupportRecord::query()->create([
                    ...
                    'Support_Type' => TeacherSupportNewTeacherDisplay::supportTypeForPayload(
                        $validated,
                        (string) config('coach_teacher_lva_fr.support_type_label'),
                    ),
            ...
            return TeacherLvaFrSupportReport::query()->create([
    ...
    private function authorize(Teacher $teacher, User $user): void
    {
        ...
        if (! $query->exists()) {
            throw new AuthorizationException('이 교사에 대한 LVA+FR 보고서를 작성할 권한이 없습니다.');
        }
    }
