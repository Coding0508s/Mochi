...
class StoreTeacherProConSupportReport
{
    /**
     * @param  array<string, mixed>  $data
     */
    public function execute(int $teacherId, array $data, User $user): TeacherProConSupportReport
    {
        ...
        return DB::transaction(function () use ($teacher, $validated, $user, $status, $markCompleted): TeacherProConSupportReport {
            ...
            if ($markCompleted) {
                ...
                $supportRecord = SupportRecord::query()->create([
                    ...
                    'Support_Type' => config('coach_teacher_pro_con.support_type_label'),
                ...
                TeacherSupportSlotSync::apply(
                    ...
                    (string) config('coach_teacher_pro_con.support_type_label'),
                );
            }

            return TeacherProConSupportReport::query()->create([
    ...
    private function authorize(Teacher $teacher, User $user): void
    {
        ...
        if (! $query->exists()) {
            throw new AuthorizationException('이 교사에 대한 Pro Con 보고서를 작성할 권한이 없습니다.');
        }
    }
