    ...
    <div class="mochi-modal-overlay {{ $viewOnly ? 'z-[70]' : 'z-[60]' }}" wire:click.self="closeUnit31PlusModal">
        <div class="mochi-modal-shell max-w-5xl max-h-[95vh] min-h-0 flex flex-col" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 border-b bg-gradient-to-r from-blue-50/80 to-white shrink-0">
                <div>
                    <h3 class="text-lg font-semibold text-blue-700">{{ $unit31PlusConfig['modal_title'] ?? 'TR 교사지원U31' }}</h3>
                    <p class="text-xs text-gray-500 mt-0.5">{{ $unit31PlusConfig['section_title'] ?? 'Unit 31+ (Training)' }}</p>
                </div>
                <button type="button" wire:click="closeUnit31PlusModal"
