@if($showUnit31PlusModal)
    @php $viewOnly = $supportReportViewMode ?? false; @endphp
    <div class="mochi-modal-overlay {{ $viewOnly ? 'z-[70]' : 'z-[60]' }}" wire:click.self="closeUnit31PlusModal">
        <div class="mochi-modal-shell max-w-5xl max-h-[95vh] min-h-0 flex flex-col" @click.stop>
            <x-admin.modal-header
                :title="$unit31PlusConfig['modal_title'] ?? 'TR 교사지원U31'"
                :subtitle="$unit31PlusConfig['section_title'] ?? 'Unit 31+ (Training)'"
                close-action="closeUnit31PlusModal"
            />
            ...
            <div class="mochi-modal-body-scroll px-6 py-4">
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    <div class="px-4 py-2.5 bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-800">
                        {{ $unit31PlusConfig['section_title'] ?? 'Unit 31+ (Training)' }}
                    </div>
                    <div class="p-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="space-y-3">
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">담당 코치</label>
                                <input type="text" wire:model="unit31PlusForm.coach_name" readonly
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="unit31PlusForm.institution_name" readonly
                                       class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">교사명</label>
                                <input type="text" wire:model="unit31PlusForm.teacher_name" readonly
                                       class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">작성날짜</label>
                                <input type="date" wire:model="unit31PlusForm.support_date"
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <select wire:model="unit31PlusForm.teacher_experience"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                    @foreach($unit31PlusConfig['teacher_experience_options'] ?? [] as $option)
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <select wire:model="unit31PlusForm.session_number"
                                            class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                        @foreach($unit31PlusConfig['session_options'] ?? [] as $session)
                                            <option value="{{ $session }}">{{ $session }} 차</option>
                                        @endforeach
                                    </select>
                                    <select wire:model="unit31PlusForm.semester_label"
                                            class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                        @foreach($unit31PlusConfig['semester_options'] ?? [] as $semester)
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <input type="date" wire:model="unit31PlusForm.interview_date"
                                           class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                    <input type="time" wire:model="unit31PlusForm.interview_time"
                            ...
                            <div>
                                ...
                                <select wire:model="unit31PlusForm.method"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                    @foreach($unit31PlusConfig['method_options'] ?? [] as $method)
                            ...
                            <div class="border border-dashed border-blue-200 rounded-lg p-3 bg-blue-50/30">
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <input type="number" min="0" max="99" wire:model="unit31PlusForm.progress_unit"
                                           class="w-14 px-2 py-1 text-sm border border-gray-300 rounded-lg text-center focus:ring-2 focus:ring-mochi-header">
                                    <span class="text-xs text-gray-600">Lesson</span>
                                    <input type="number" min="0" max="99" wire:model="unit31PlusForm.progress_lesson"
                                           class="w-14 px-2 py-1 text-sm border border-gray-300 rounded-lg text-center focus:ring-2 focus:ring-mochi-header">
                                    <span class="text-xs text-gray-600">Other</span>
                                    <input type="text" wire:model="unit31PlusForm.progress_other"
                                           class="flex-1 min-w-[8rem] px-2 py-1 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($unit31PlusConfig['procedures'] ?? [] as $key => $procedure)
                            <div class="space-y-2">
                                <label class="flex items-start gap-2 text-sm font-medium text-gray-800 cursor-pointer">
                                    <input type="checkbox" value="{{ $key }}" wire:model="unit31PlusForm.procedures"
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div class="p-4 space-y-4">
                        @php
                            $verbalSection = $unit31PlusConfig['materials']['verbal_skill_tools'] ?? [];
                            $languageSection = $unit31PlusConfig['materials']['language_arts_tools'] ?? [];
                        @endphp
                                ...
                                <div class="space-y-2">
                                    @foreach($verbalSection['items'] ?? [] as $itemKey => $itemLabel)
                                        <label class="flex items-start gap-2 text-sm text-gray-700 cursor-pointer">
                                            <input type="checkbox" value="{{ $itemKey }}" wire:model="unit31PlusForm.verbal_materials"
                                                   class="mt-0.5 rounded border-gray-300 text-mochi-header focus:ring-mochi-header">
                                            <span>{{ $itemLabel }}</span>
                                        </label>
                                    @endforeach
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    @foreach($languageSection['items'] ?? [] as $itemKey => $itemLabel)
                                        <label class="flex items-start gap-2 text-sm text-gray-700 cursor-pointer">
                                            <input type="checkbox" value="{{ $itemKey }}" wire:model="unit31PlusForm.language_arts_materials"
                                                   class="mt-0.5 rounded border-gray-300 text-mochi-header focus:ring-mochi-header">
                                            <span>{{ $itemLabel }}</span>
                                        </label>
                                    @endforeach
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <textarea wire:model="unit31PlusForm.verbal_comments" rows="3"
                                          class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header resize-y"></textarea>
                                <textarea wire:model="unit31PlusForm.language_arts_comments" rows="3"
                                          class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header resize-y"></textarea>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea wire:model="unit31PlusForm.overall_comments" rows="3"
                                      class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header resize-y"></textarea>
                        </div>
                    </div>
                </section>
            ...
            <div class="shrink-0 @if($viewOnly) pointer-events-auto @endif">
                @include('components.coach.partials.support-report-modal-footer', [
                    'closeAction' => 'closeUnit31PlusModal',
                    'saveAction' => 'saveUnit31PlusReport',
                    'markCompletedModel' => 'unit31PlusMarkCompleted',
                ])
            </div>
        </div>
    </div>
