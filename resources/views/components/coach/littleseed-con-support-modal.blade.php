    ...
    <div class="mochi-modal-overlay {{ $viewOnly ? 'z-[70]' : 'z-[60]' }}" wire:click.self="closeLittleseedConModal">
        <div class="mochi-modal-shell max-w-5xl max-h-[95vh] min-h-0 flex flex-col" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 border-b bg-gradient-to-r from-blue-50/80 to-white shrink-0">
                <div>
                    <h3 class="text-lg font-semibold text-blue-700">{{ $littleseedConConfig['modal_title'] ?? 'TR 교사지원LSPConsulting' }}</h3>
                    <p class="text-xs text-gray-500 mt-0.5">{{ $littleseedConConfig['section_title'] ?? 'LittleSEED Consulting' }}</p>
                </div>
                <button type="button" wire:click="closeLittleseedConModal"
