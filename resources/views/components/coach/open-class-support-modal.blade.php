@if($showOpenClassModal)
    @php $viewOnly = $supportReportViewMode ?? false; @endphp
    <div class="mochi-modal-overlay {{ $viewOnly ? 'z-[70]' : 'z-[60]' }}" wire:click.self="closeOpenClassModal">
        <div class="mochi-modal-shell max-w-5xl max-h-[95vh] min-h-0 flex flex-col" @click.stop>
            <x-admin.modal-header
                :title="$openClassConfig['modal_title'] ?? 'TR 교사지원OpenClass'"
                :subtitle="$openClassConfig['section_title'] ?? 'Open-Class'"
                close-action="closeOpenClassModal"
            />
            ...
            <div class="mochi-modal-body-scroll px-6 py-4">
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    <div class="px-4 py-2.5 bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-800">
                        {{ $openClassConfig['section_title'] ?? 'Open-Class' }}
                    </div>
                    <div class="p-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="space-y-3">
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">담당 코치</label>
                                <input type="text" wire:model="openClassForm.coach_name" readonly
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="openClassForm.institution_name" readonly
                                       class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">교사명</label>
                                <input type="text" wire:model="openClassForm.teacher_name" readonly
                                       class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">작성날짜</label>
                                <input type="date" wire:model="openClassForm.support_date"
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <select wire:model="openClassForm.teacher_experience"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                    @foreach($openClassConfig['teacher_experience_options'] ?? [] as $option)
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <select wire:model="openClassForm.session_number"
                                        ...
                                        @foreach($openClassConfig['session_options'] ?? [] as $session)
                                            <option value="{{ $session }}">{{ $session }} 차</option>
                                        @endforeach
                                    </select>
                                    <select wire:model="openClassForm.semester_label"
                                            class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                        @foreach($openClassConfig['semester_options'] ?? [] as $semester)
                            ...
                            <div>
                                ...
                                <select wire:model="openClassForm.method"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-mochi-header">
                                    @foreach($openClassConfig['method_options'] ?? [] as $method)
                            ...
                            <div class="border border-dashed border-blue-200 rounded-lg p-3 bg-blue-50/30">
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <input type="number" min="0" max="99" wire:model="openClassForm.progress_unit"
                                    ...
                                    <input type="number" min="0" max="99" wire:model="openClassForm.progress_lesson"
                                           class="w-14 px-2 py-1 text-sm border border-gray-300 rounded-lg text-center focus:ring-2 focus:ring-mochi-header">
                                    <span class="text-xs text-gray-600">Other</span>
                                    <input type="text" wire:model="openClassForm.progress_other"
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach($openClassConfig['procedures'] ?? [] as $key => $label)
                            <label class="flex items-start gap-2 text-sm text-gray-700 cursor-pointer">
                                <input type="checkbox" value="{{ $key }}" wire:model="openClassForm.procedures"
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    <div class="px-4 py-2.5 bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-800">지원 내용</div>
                    <div class="p-4 space-y-2">
                        @foreach($openClassConfig['support_content'] ?? [] as $key => $label)
                            <label class="flex items-start gap-2 text-sm text-gray-700 cursor-pointer">
                                <input type="checkbox" value="{{ $key }}" wire:model="openClassForm.support_content"
                                       class="mt-0.5 rounded border-gray-300 text-mochi-header focus:ring-mochi-header">
                                <span>{{ $label }}</span>
                            </label>
                        @endforeach
                ...
                <section class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                    <div class="px-4 py-2.5 bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-800">비고</div>
                    <div class="p-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">비고:</label>
                        <textarea wire:model="openClassForm.remarks" rows="4"
            ...
            <div class="shrink-0 @if($viewOnly) pointer-events-auto @endif">
                @include('components.coach.partials.support-report-modal-footer', [
                    'closeAction' => 'closeOpenClassModal',
                    'saveAction' => 'saveOpenClassReport',
                    'markCompletedModel' => 'openClassMarkCompleted',
                ])
            </div>
        </div>
    </div>
